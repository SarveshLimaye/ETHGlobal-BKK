...
contract PoolFactory is Ownable, CCIPReceiver, Register,IEntropyConsumer {
    ...
    function getPythPrice (bytes[] calldata priceUpdate,address token) public payable returns(int64 priceUint, int32 expo) {
        ...
        bytes32 priceFeed = s_addressToFeedId[token];
        PythStructs.Price memory priceData = s_pyth.getPrice(priceFeed);
    ...
    function getDeployedPoolsInCurrentChain() external view returns (address[] memory) {
        return s_allDeployedPoolsInCurrentChain;
    }

     function updateAddressToFeedId(bytes32 feedId, address asset) external  {
        s_addressToFeedId[asset] = feedId;
    }
