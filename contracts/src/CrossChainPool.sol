 ...
 *  Users can "bridge" tokens to the other networks
...
contract CrossChainPool is ERC20, ReentrancyGuard, CCIPReceiver, Ownable {
    ...
    uint8 private constant BRIDGE_FUNCTION_ID = 1;
    uint8 private constant ADD_LIQUIDITY_FUNCTION_ID = 2;
    uint8 private constant WITHDRAW_FUNCTION_ID = 3;
    ...
    event Bridge(
        uint256 indexed bridgeedAmount, address indexed to, address indexed underlyingToken, uint64 crosschainSelector
    );
     ...
     */
    modifier isLPWithdrawingCorrectAmount(uint256 _amount) {
     ...
     * @param _underlyingToken is the ERC20 that gets bridged
    ...
    ) ERC20(string.concat("ChainSync", _name), string.concat("CSX", _name)) CCIPReceiver(_router) Ownable(msg.sender) {
     ...
     */
    function addLiquidity(IERC20 _token, uint256 _amount) external payable isCorrectToken(_token) moreThanZero(_amount) {
     ...
     */
    function withdraw(uint256 _lptAmount, address _to)
        external
        payable
        nonReentrant
        isUserLiquidityProvider
        isCooldownPeriodExpired
        isLPWithdrawingCorrectAmount(_lptAmount)
    {
    ...
    /**
     * @notice users deposit here underlying tokens and send a message to the other pool crosschain that will send the bridgeed token to the _to address specified in this function
     ...
     */
    function bridge(uint256 _value, address _to) external payable  {
        address router = getRouter();

        // protocol fees
        uint256 fees = calculateProtocolFees(_value);

        // user deposit underlying tokens
        emit Bridge(_value, _to, address(i_underlyingToken), i_crossChainSelector);

        i_underlyingToken.safeTransferFrom(msg.sender, address(this), _value);

        // send a message to other pool
        _sendCCipMessageBridge(_value, fees, _to, router);
    }
     ...
     *  @return fees
     */
    function calculateProtocolFees(uint256 _value) public view returns (uint256 fees) {
     ...
     *  subtract value to bridge - fees from s_crossChainUnderlyingBalance
     *  fees are retained in destination chain
     */
    function _sendCCipMessageBridge(uint256 _value, uint256 _fees, address _to, address _router) internal {
        Client.EVM2AnyMessage memory message = _buildBridgeMessage(_value, _fees, _to);
    ...
    function _sendCCipMessageDeposit(
        ...
        Client.EVM2AnyMessage memory message = _buildAddLiquidityMessage(_underlyingDepositedAmount, _liquidityTokensMinted);
     ...
     */
    function _buildAddLiquidityMessage(uint256 _underlyingDepositedAmount, uint256 _liquidityTokensMinted)
        internal
        view
        returns (Client.EVM2AnyMessage memory message)
    {
        message = Client.EVM2AnyMessage({
            ...
            data: abi.encode(ADD_LIQUIDITY_FUNCTION_ID, _underlyingDepositedAmount, _liquidityTokensMinted), //  the parameters to pass into deployPool
        ...
        returns (Client.EVM2AnyMessage memory message)
    {
        message = Client.EVM2AnyMessage({
            receiver: abi.encode(s_crossChainPool), // receiver is the pool on destination chain
            data: abi.encode(WITHDRAW_FUNCTION_ID, _amountToRedeem, _amountRedeemedOnSourceChain, _to), //  the parameters to pass into deployPool
            tokenAmounts: new Client.EVMTokenAmount[](0), // we are not passing tokens even tho we bridge bc we cool AF
            extraArgs: Client._argsToBytes(
                // Additional arguments, setting gas limit
                Client.EVMExtraArgsV1({gasLimit: s_gas_limit})
    ...
    /**
     * @notice build the bridge message
     */
    function _buildBridgeMessage(uint256 _value, uint256 _fees, address _to)
        internal
        view
        returns (Client.EVM2AnyMessage memory message)
    {
        message = Client.EVM2AnyMessage({
            receiver: abi.encode(s_crossChainPool),
            data: abi.encode(BRIDGE_FUNCTION_ID, _value, _fees, _to),
            tokenAmounts: new Client.EVMTokenAmount[](0), // we are not passing tokens even tho we bridge bc we cool AF
            extraArgs: Client._argsToBytes(
                // Additional arguments, setting gas limit
                Client.EVMExtraArgsV1({gasLimit: s_gas_limit})
     ...
     * @notice build the bridge message
     */
    function _simulateMessageBridge(uint256 _value, address _to)
        internal
        view
        returns (Client.EVM2AnyMessage memory message)
    {
        uint256 fees = calculateProtocolFees(_value);
        message = _buildBridgeMessage(_value, fees, _to);
    }

    /**
     * @notice simulate the deposit message - needed to evaluate the cost of deposit in ccip fees
     */
    function _simulateMessageAddLiquidity(uint256 _underlyingDepositedAmount)
        internal
        view
        returns (Client.EVM2AnyMessage memory message)
    {
        ...
        message = _buildAddLiquidityMessage(_underlyingDepositedAmount, lptToMint);
    }
        ...
        areSenderAndSelectorAllowed(any2EvmMessage.sender, any2EvmMessage.sourceChainSelector)
    {
        ...
        if (functionID == BRIDGE_FUNCTION_ID) {
            (, uint256 bridgeedValue, uint256 feesForbridgeing, address to) =
                abi.decode(any2EvmMessage.data, (uint8, uint256, uint256, address));

            s_crossChainUnderlyingBalance += bridgeedValue; //should be + fees
            i_underlyingToken.safeTransfer(to, bridgeedValue - feesForbridgeing);
        }
        if (functionID == ADD_LIQUIDITY_FUNCTION_ID) {
        ...
        if (functionID == WITHDRAW_FUNCTION_ID) {
    ...
    /**
     * @notice this is used with bridge function to forecast the ccip fees and let the user pay those fees
     */
    function getCcipFeesForBridging(uint256 _value, address _to) public view returns (uint256) {
        address router = getRouter();
        Client.EVM2AnyMessage memory message = _simulateMessageBridge(_value, _to);
    ...
    function getCCipFeesForDeposit(uint256 _value) public view returns (uint256) {
        address router = getRouter();
        Client.EVM2AnyMessage memory message = _simulateMessageAddLiquidity(_value);
        return IRouterClient(router).getFee(i_crossChainSelector, message);
    }
